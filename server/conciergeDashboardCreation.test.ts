...
describe("Concierge dashboard creation", () => {
  it("uses one conversational question at a time for guest account creation", () => {
    expect(source).toContain("Let me gather a few details and create your DreamCarz dashboard. What email should we use?");
  ...
  it("keeps password entry masked and offers a question interruption path", () => {
    expect(source).toContain('type={(dashboardCreationField === "password" || dashboardCreationField === "existingPassword") && !dashboardQuestionMode ? "password" : "text"}');
