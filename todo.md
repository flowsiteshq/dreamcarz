...
- [x] Surface existing transaction-bound additional-driver records in Concierge so they remain pending separate review and do not imply rental eligibility clearance.
- [x] Add an administrator-only additional-driver review view showing name and review statuses without contact details, documents, or automatic eligibility clearance.
...
- [x] Require an active administrator-controlled eligibility policy before a rental transaction can be manually cleared for eligibility.
...
- [x] Apply bounded per-administrator rate limits before private Vehicle Passport record lists, directory results, and signed document links are read.
...
- [x] Record safe Vehicle Passport readiness-status transitions in the private activity timeline without customer, location, document, or financial data.
...
- [x] Reject likely payment-card numbers, passwords, and labeled driver-license values before authorized administrator extension, settlement, document, or reservation review notes are stored.
...
- [x] Add a privacy-minimized recent wallet-activity panel to the Associate portal using only account-owned ledger type, status, amount, and timestamp data.
- [x] Audit and correct visible Associate portal actions so labels match their protected function and buttons route to working portal destinations.
...
- [x] Remove non-functional account profile edit controls until protected profile updates are available.
...
- [x] Add a direct Users and roles shortcut to the Administrator overview quick-access panel.
