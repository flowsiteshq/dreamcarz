...
const confirmedInventory = [
  { id: "2024-chevrolet-malibu-gray", year: 2024, make: "Chevrolet", model: "Malibu", color: "Gray", type: "Sedan", access: "mid-range", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/ogLykrxMFWpmsTbU.png" },
  { id: "2022-chevrolet-traverse-white", year: 2022, make: "Chevrolet", model: "Traverse", color: "White", type: "SUV", access: "elite", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/uLwJSHBxRyWslZQQ.png" },
  { id: "2024-ford-fusion-gray", year: 2024, make: "Ford", model: "Fusion", color: "Gray", type: "Sedan", access: "mid-range", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/qRKjtXjkrFUfxMqh.png" },
  { id: "2020-chevrolet-traverse-gray", year: 2020, make: "Chevrolet", model: "Traverse", color: "Gray", type: "SUV", access: "mid-range", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/mFFrLJBzyJWqricP.png" },
  { id: "2019-chevrolet-malibu-black", year: 2019, make: "Chevrolet", model: "Malibu", color: "Black", type: "Sedan", access: "entry", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/njSpzrWxcQZbiWBb.png" },
  { id: "2015-ford-taurus-gray", year: 2015, make: "Ford", model: "Taurus", color: "Gray", type: "Sedan", access: "entry", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/BDcxxQLQxENUVvqj.png" },
  { id: "2020-chevrolet-equinox-gray", year: 2020, make: "Chevrolet", model: "Equinox", color: "Gray", type: "SUV", access: "entry", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/QuCANehFYgQjJKfm.png" },
  { id: "2020-chevrolet-equinox-black", year: 2020, make: "Chevrolet", model: "Equinox", color: "Black", type: "SUV", access: "entry", image: "https://files.manuscdn.com/user_upload_by_module/session_file/310419663031545745/TAiKEadRDaSWeYcf.png" },
] as const;
...
const planPricing = {
  free: { name: "Free", enrollment: "$0", monthly: "No monthly fee" },
  freedom: { name: "Freedom", enrollment: "$199", monthly: "$39 monthly" },
  plus: { name: "Plus", enrollment: "$499", monthly: "$99 monthly" },
  pro: { name: "Pro", enrollment: "$1,499", monthly: "$149 monthly" },
  elite: { name: "Elite", enrollment: "$2,965", monthly: "$249 monthly" },
  silver: { name: "Silver", enrollment: "$4,785", monthly: "$399 monthly" },
  gold: { name: "Gold", enrollment: "$9,985", monthly: "$699 monthly" },
  black: { name: "Black", enrollment: "$24,950", monthly: "$1,250 monthly" },
} as const;
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      ...
      <main>
        ...
        <section className="mx-auto max-w-7xl px-6 py-14 lg:px-10">{selectedMembershipPlan && <div className="mb-8 flex flex-col gap-3 rounded-2xl border border-[#dbc99a] bg-[#fcfaf2] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Your selected membership</p><p className="mt-1 text-sm text-gray-700"><strong className="text-black">{selectedMembershipPlan.name}</strong> · {selectedMembershipPlan.enrollment} enrollment · {selectedMembershipPlan.monthly}. <span className="text-gray-500">Vehicle costs remain separate and are reviewed when you choose Rent or Buy.</span></p></div><Link href="/pricing" className="shrink-0 text-sm font-semibold underline underline-offset-4">Change plan</Link></div>}<div className="flex flex-col justify-between gap-5 border-b border-gray-200 pb-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><p className="text-sm text-gray-500">{inventory.length} of {confirmedInventory.length} confirmed vehicles{accessFilter !== "all" ? ` in the ${accessFilter === "mid-range" ? "Mid-Range" : accessFilter[0].toUpperCase() + accessFilter.slice(1)} access view` : ""}</p><div className="flex items-center gap-2"><Filter size={15} className="text-gray-400" />{filters.map((item) => <button type="button" key={item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${filter === item ? "bg-black text-white" : "border border-gray-200 text-gray-600 hover:border-black hover:text-black"}`}>{item}</button>)}</div></div><div className="flex flex-wrap gap-2">{accessFilters.map((access) => <button type="button" key={access} onClick={() => setAccessFilter(access)} className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${accessFilter === access ? "bg-[#a8832d] text-white" : "border border-gray-200 text-gray-600"}`}>{access === "all" ? "All vehicle access" : access === "mid-range" ? "Mid-Range" : `${access[0].toUpperCase()}${access.slice(1)} access`}</button>)}</div></div>
      ...
      <Footer />
      {selectedVehicle && <VehicleExperienceDialog vehicle={selectedVehicle.vehicle} membershipPlan={selectedMembershipPlan} open={Boolean(selectedVehicle)} initialView={selectedVehicle.view} onOpenChange={(open) => !open && setSelectedVehicle(null)} />}
    </div>
  );
}
