...
import { COMING_SOON_VEHICLE_CATEGORIES, comingSoonVehicleCatalog } from "@shared/comingSoonVehicles";
...
const comingSoonVehicles = comingSoonVehicleCatalog;
const filters = ["All", "Sedan", "SUV", ...COMING_SOON_VEHICLE_CATEGORIES.filter((category) => category !== "Sedan" && category !== "SUV")] as const;
...
function VehicleCard({ vehicle, onSelect }: { vehicle: FleetVehicle; onSelect: (vehicle: FleetVehicle, view: "overview" | "rental" | "purchase" | "reserve") => void }) {
  ...
  return <article className="group border-b border-gray-200 py-7 sm:px-6 sm:odd:border-l lg:border-b-0 lg:border-l lg:first:border-l-0">
    <button type="button" onClick={() => onSelect(vehicle, "overview")} className="block w-full text-left focus:outline-none">
      ...
      <p className="mt-3 text-sm text-gray-500">{comingSoon ? "Illustrative category visual · exterior to be confirmed" : `Exterior: ${vehicle.color}`}</p>
...
export default function Fleet() {
  ...
  return <div className="min-h-screen bg-white text-black"><Navigation /><main>
    ...
    <section className="mx-auto max-w-7xl px-6 py-14 lg:px-10">{selectedMembershipPlan && <div className="mb-8 flex flex-col gap-3 rounded-2xl border border-[#dbc99a] bg-[#fcfaf2] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Your selected membership</p><p className="mt-1 text-sm text-gray-700"><strong className="text-black">{selectedMembershipPlan.name}</strong> · {selectedMembershipPlan.focus}. <span className="text-gray-500">Membership and vehicle-specific terms are reviewed separately when you choose Rent, Buy, or Reserve.</span></p></div><Link href="/pricing" className="shrink-0 text-sm font-semibold underline underline-offset-4">Change plan</Link></div>}
      ...
      {reserveVehicles.length > 0 && <div className="mt-16 border-t border-gray-200 pt-10"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#a8832d]">Expand your path</p><h2 className="mt-2 font-display text-3xl font-bold tracking-[-0.04em]">Coming Soon · join the waiting list</h2></div><p className="max-w-xl text-sm leading-6 text-gray-500">{comingSoonVehicles.length} planned options across {COMING_SOON_VEHICLE_CATEGORIES.length} categories. These are not part of current DreamCarz inventory; images are illustrative category visuals, and every interest request is reviewed before any next step.</p></div><div className="mt-6 grid grid-cols-1 border-t border-gray-200 sm:grid-cols-2 lg:grid-cols-4">{reserveVehicles.map((vehicle) => <VehicleCard key={vehicle.id} vehicle={vehicle} onSelect={(selected, view) => setSelectedVehicle({ vehicle: selected, view })} />)}</div></div>}
