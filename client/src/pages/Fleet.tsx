...
const planPricing = {
  free: { name: "Free", enrollment: "$0", monthly: "No monthly fee" }, freedom: { name: "Freedom", enrollment: "$199", monthly: "$39 monthly" }, plus: { name: "Plus", enrollment: "$499", monthly: "$99 monthly" }, pro: { name: "Pro", enrollment: "$1,499", monthly: "$149 monthly" }, elite: { name: "Elite", enrollment: "$2,965", monthly: "$249 monthly" }, silver: { name: "Silver", enrollment: "$4,785", monthly: "$399 monthly" }, gold: { name: "Gold", enrollment: "$9,985", monthly: "$699 monthly" }, black: { name: "Black", enrollment: "$24,950", monthly: "$1,250 monthly" },
} as const;
...
export default function Fleet() {
  ...
  return <div className="min-h-screen bg-white text-black"><Navigation /><main>
    ...
    <section className="mx-auto max-w-7xl px-6 py-14 lg:px-10">{selectedMembershipPlan && <div className="mb-8 flex flex-col gap-3 rounded-2xl border border-[#dbc99a] bg-[#fcfaf2] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Your selected membership</p><p className="mt-1 text-sm text-gray-700"><strong className="text-black">{selectedMembershipPlan.name}</strong> · {selectedMembershipPlan.enrollment} enrollment · {selectedMembershipPlan.monthly}. <span className="text-gray-500">Vehicle costs remain separate and are reviewed when you choose Rent, Buy, or Reserve.</span></p></div><Link href="/pricing" className="shrink-0 text-sm font-semibold underline underline-offset-4">Change plan</Link></div>}
