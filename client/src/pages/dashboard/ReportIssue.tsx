        ...
        {tab === "resources" && (
          <div className="space-y-3">
            <div className="bg-white rounded-2xl border border-gray-100 p-4">
              ...
              {reportHistory.data?.length ? reportHistory.data.slice(0, 3).map(report => <div key={report.id} className="py-3 border-b border-gray-50 last:border-0"><div className="flex items-center justify-between"><div><p className="text-[12px] font-semibold text-black">{report.category}</p><p className="text-[10px] text-gray-400">{report.reference}</p></div><span className="text-[10px] font-bold uppercase text-gray-500">{report.status.replaceAll("_", " ")}</span></div><div className="mt-2 border-l border-gray-200 pl-3 space-y-1">{report.history?.map(event => <p key={event.id} className="text-[10px] text-gray-400"><span className="font-semibold text-black">{event.status.replaceAll("_", " ")}</span>{event.note ? ` · ${event.note}` : ""}</p>)}</div></div>) : <p className="text-[12px] text-gray-400">No submitted service reports yet.</p>}
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50 overflow-hidden">
