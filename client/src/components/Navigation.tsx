...
type MegaMenuKey = "fleet" | "members" | "pricing" | "network";
...
export default function Navigation() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isConciergeWorkspace ? "border-b border-white/10 bg-black text-white" : scrolled || activeMegaMenu ? "bg-white/95 backdrop-blur-md shadow-[0_1px_0_rgba(0,0,0,0.05)]" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-center justify-between h-[68px]">
          ...
          <nav className="hidden lg:flex items-center gap-1" aria-label="Primary navigation" onMouseLeave={() => setActiveMegaMenu(null)} onBlur={closeWhenFocusLeaves}>
            ...
            <Link href="/pricing" aria-expanded={activeMegaMenu === "pricing"} aria-controls="dreamcarz-pricing-menu" onMouseEnter={() => setActiveMegaMenu("pricing")} onFocus={() => setActiveMegaMenu("pricing")} onKeyDown={event => { if (event.key === "Escape") setActiveMegaMenu(null); }} className={triggerClassName("pricing")}>Pricing <ChevronDown size={14} className={`transition-transform ${activeMegaMenu === "pricing" ? "rotate-180" : ""}`} /></Link>
      ...
      {activeMegaMenu === "pricing" ? <div id="dreamcarz-pricing-menu" className="absolute left-0 right-0 top-full hidden border-t border-[#ece6da] bg-white shadow-[0_22px_44px_rgba(0,0,0,0.12)] lg:block" onMouseEnter={() => setActiveMegaMenu("pricing")} onMouseLeave={() => setActiveMegaMenu(null)}>
        <div className="mx-auto grid max-w-7xl grid-cols-[0.86fr_1.14fr] gap-12 px-10 py-8"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#a8832d]">Pricing with clarity</p><h2 className="mt-2 max-w-sm text-2xl font-semibold tracking-[-0.04em] text-black">Keep membership context and vehicle access in their own lanes.</h2><p className="mt-4 max-w-sm text-sm leading-6 text-gray-500">Review program paths first, then request a vehicle-specific estimate with the relevant details.</p><Link href="/pricing" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-black underline decoration-[#b08b35] decoration-2 underline-offset-4">Open pricing <ChevronDown className="-rotate-90" size={16} /></Link></div><div className="grid grid-cols-3 gap-4"><Link href="/pricing" className="border border-[#ece6da] p-5 hover:border-[#bfa35d]"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Compare</p><p className="mt-3 text-sm font-semibold text-black">Membership paths</p><p className="mt-2 text-xs leading-5 text-gray-500">Review recurring membership context separately.</p></Link><Link href="/concierge?intent=rental" className="border border-[#ece6da] p-5 hover:border-[#bfa35d]"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Rent</p><p className="mt-3 text-sm font-semibold text-black">Vehicle estimate</p><p className="mt-2 text-xs leading-5 text-gray-500">Select a confirmed vehicle and plan the next step.</p></Link><Link href="/concierge?intent=purchase" className="border border-[#ece6da] p-5 hover:border-[#bfa35d]"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#a8832d]">Buy</p><p className="mt-3 text-sm font-semibold text-black">Purchase path</p><p className="mt-2 text-xs leading-5 text-gray-500">Explore an available purchase journey with Concierge.</p></Link></div></div>
      </div> : null}
