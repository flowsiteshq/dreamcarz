...
function RentalConditionReportPanel({ reference }: { reference: string }) {
  ...
  const stageEvidence = (transaction.data?.conditionEvidence ?? []).filter(item => item.stage === stage);
  const storedViews = new Set(stageEvidence.map(item => item.view));
