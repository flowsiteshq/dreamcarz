...
import { ArrowLeft, CalendarDays, CarFront, CheckCircle2, Mail, Phone, ShoppingBag, X } from "lucide-react";
...
type ViewState = "overview" | "rental" | "purchase" | "success";
...
export function VehicleExperienceDialog({ vehicle, open, onOpenChange, initialView = "overview" }: VehicleExperienceDialogProps) {
  ...
  const submitInquiry = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isRental && !isPurchase) return;
    createInquiry.mutate({
      inquiryType: isRental ? "rental" : "purchase",
        ...
        {view === "overview" && (
          <div className="grid min-h-[720px] lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="flex flex-col justify-between bg-[#f4f1e9] px-8 py-14 sm:px-12">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#a8832d]">DreamCarz confirmed inventory</p>
                ...
                <DialogDescription className="mt-5 max-w-md text-base leading-relaxed text-gray-600">{vehicle.type} · Exterior: {vehicle.color}. Review the vehicle in full, then choose whether you want to request a rental or ask about purchase options.</DialogDescription>
                <div className="mt-10 grid gap-3 text-sm text-gray-600">
                  <p className="border-l-2 border-[#a8832d] pl-4">Availability and final terms are confirmed after your request is reviewed.</p>
              ...
              <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                <button type="button" onClick={() => setView("rental")} className="inline-flex h-12 items-center justify-center gap-2 bg-black px-5 text-sm font-semibold text-white transition-transform duration-150 active:scale-[0.97]"><CalendarDays size={16} /> Rent this vehicle</button>
                <button type="button" onClick={() => setView("purchase")} className="inline-flex h-12 items-center justify-center gap-2 border border-black bg-transparent px-5 text-sm font-semibold text-black transition-colors hover:bg-white"><ShoppingBag size={16} /> Buy this vehicle</button>
              </div>
            </div>
          </div>
        )}
        ...
        {(isRental || isPurchase) && (
          <div className="grid min-h-[720px] lg:grid-cols-[0.78fr_1.22fr]">
            <aside className="hidden bg-black px-10 py-14 text-white lg:block">
              ...
              <p className="mt-10 text-[11px] font-bold uppercase tracking-[0.2em] text-[#d1ad54]">{isRental ? "Rental request" : "Purchase inquiry"}</p>
              <h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.045em]">{fullName}</h2>
              <p className="mt-4 text-sm leading-relaxed text-white/65">{isRental ? "Share your preferred dates and location. DreamCarz will review your request and guide you through any remaining rental requirements." : "Share your preferred contact details. DreamCarz will follow up with the vehicle’s current purchase options and next steps."}</p>
            </aside>
            <div className="px-6 py-16 sm:px-12">
              ...
              <DialogHeader className="mt-8 text-left lg:mt-0">
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#a8832d]">{isRental ? "Request a rental" : "Ask about purchase"}</p>
              ...
              <form onSubmit={submitInquiry} className="mt-9 grid gap-5">
                ...
                <label className="grid gap-2 text-sm font-semibold text-black">Anything else we should know?<textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="min-h-24 w-full resize-y border border-gray-200 bg-white p-3 text-sm text-black outline-none placeholder:text-gray-400 focus:border-black" placeholder={isRental ? "Rental purpose or timing notes" : "Questions about purchase options"} /></label>
                {createInquiry.error && <p className="text-sm text-red-600">{createInquiry.error.message}</p>}
                <button disabled={createInquiry.isPending} type="submit" className="inline-flex h-12 items-center justify-center gap-2 bg-black px-6 text-sm font-semibold text-white transition-transform duration-150 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"><CarFront size={16} /> {createInquiry.isPending ? "Sending request…" : isRental ? "Submit rental request" : "Submit purchase inquiry"}</button>
              </form>
            </div>
          </div>
        )}
        ...
        {view === "success" && (
          <div className="flex min-h-[560px] items-center justify-center px-8 py-16 text-center">
            <div className="max-w-lg">
              ...
              <DialogDescription className="mt-5 text-base leading-relaxed text-gray-600">DreamCarz received your {submittedInquiryType === "rental" ? "rental request" : "purchase inquiry"} for the {fullName}. We will use your selected contact method to confirm the next step.</DialogDescription>
