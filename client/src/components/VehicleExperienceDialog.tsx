...
export function VehicleExperienceDialog({ vehicle, membershipPlan, open, onOpenChange, initialView = "overview" }: VehicleExperienceDialogProps) {
  ...
  const isTeslaModel3Waitlist = vehicle.id === "coming-soon-2024-tesla-model-3";
  const reserveLabel = isComingSoon ? `Join ${fullName} waiting list` : "Reserve your vehicle";
        ...
        {view === "overview" && (
          <div className="grid min-h-[720px] lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="flex flex-col justify-between bg-[#f4f1e9] px-8 py-14 sm:px-12">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#a8832d]">{isComingSoon ? "Coming Soon · waiting list" : "DreamCarz confirmed inventory"}</p>
                ...
                <DialogDescription className="mt-5 max-w-md text-base leading-relaxed text-gray-600">{vehicle.type} · Exterior: {vehicle.color}. {isComingSoon ? "This planned vehicle is not current DreamCarz inventory. Its image is illustrative; join the waiting list and our team will guide you when an appropriate path is available." : "Review the vehicle in full, then choose whether you want to request a rental or ask about purchase options."}</DialogDescription>
                <div className="mt-10 grid gap-3 text-sm text-gray-600">
                  <p className="border-l-2 border-[#a8832d] pl-4">{isComingSoon ? "Waiting-list interest is reviewed before DreamCarz confirms timing, availability, or any next steps." : "Availability and final terms are confirmed after your request is reviewed."}</p>
        ...
        {isReserve && (
          <div className="grid min-h-[720px] lg:grid-cols-[0.78fr_1.22fr]">
            <aside className="hidden bg-black px-10 py-14 text-white lg:block">
              ...
              <p className="mt-10 text-[11px] font-bold uppercase tracking-[0.2em] text-[#d1ad54]">{isRental ? "Rental request" : isPurchase ? "Purchase inquiry" : isComingSoon ? "Vehicle waiting list" : "Reserve request"}</p>
              <h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.045em]">{fullName}</h2>
              <p className="mt-4 text-sm leading-relaxed text-white/65">{isRental ? "Share your preferred dates and location. DreamCarz will review your request and guide you through any remaining rental requirements." : isPurchase ? "Share your preferred contact details. DreamCarz will follow up with the vehicle’s current purchase options and next steps." : isComingSoon ? `Share your contact details to join the ${fullName} waiting list. DreamCarz will follow up when an appropriate vehicle-access path is available.` : "Share your contact details to reserve your interest. DreamCarz will follow up when an appropriate vehicle-access path is available."}</p>
            </aside>
            <div className="px-6 py-16 sm:px-12">
              ...
              <DialogHeader className="mt-8 text-left lg:mt-0">
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#a8832d]">{isRental ? "Request a rental" : isPurchase ? "Ask about purchase" : isComingSoon ? "Vehicle waiting list" : "Reserve your vehicle"}</p>
                <DialogTitle className="font-display text-4xl font-bold tracking-[-0.05em] text-black">{isRental ? "Start your rental request." : isPurchase ? "Send a purchase inquiry." : isComingSoon ? `Join the ${fullName} waiting list.` : "Reserve your vehicle."}</DialogTitle>
                <DialogDescription className="max-w-xl leading-relaxed text-gray-600">{isRental ? "We will confirm vehicle availability, then help you complete the required rental steps." : isPurchase ? "We will confirm current availability and discuss the applicable purchase path directly with you." : isComingSoon ? `${fullName} is Coming Soon. Submit your interest and DreamCarz will guide you when a suitable vehicle-access path is available; timing, availability, and final terms remain subject to confirmation.` : "This vehicle is labeled Coming Soon. Submit your interest and DreamCarz will guide you when a suitable option is ready."}</DialogDescription>
              </DialogHeader>
              ...
              <form onSubmit={submitInquiry} className="mt-9 grid gap-5">
                ...
                <button disabled={createInquiry.isPending} type="submit" className="inline-flex h-12 items-center justify-center gap-2 bg-black px-6 text-sm font-semibold text-white transition-transform duration-150 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"><CarFront size={16} /> {createInquiry.isPending ? "Sending request…" : isRental ? "Submit rental request" : isPurchase ? "Submit purchase inquiry" : reserveLabel}</button>
              </form>
            </div>
          </div>
        )}
