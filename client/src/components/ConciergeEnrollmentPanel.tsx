...
export function ConciergeEnrollmentPanel({ reference, onProgress }: { reference: string; onProgress: (message: string) => void }) {
  ...
  const addAdditionalDriver = trpc.transactions.addAdditionalDriver.useMutation({ onSuccess: () => utils.transactions.get.invalidate({ reference }) });
  ...
  const [additionalDriver, setAdditionalDriver] = useState({ fullName: "", email: "" });
  ...
  const submitAdditionalDriver = async () => {
    if (additionalDriver.fullName.trim().length < 2) { setError("Enter the additional driver’s legal name before saving."); return; }
    try {
      setError("");
      await addAdditionalDriver.mutateAsync({ reference, fullName: additionalDriver.fullName.trim(), email: additionalDriver.email.trim() || undefined });
      setAdditionalDriver({ fullName: "", email: "" });
      onProgress("Your additional driver is saved for separate review. They are not cleared to operate the vehicle until DreamCarz completes that review.");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "We could not save that additional driver yet."); }
  };
  ...
  if (transaction.currentStep === "trade_in" && transaction.transactionType !== "purchase") return <div className="mt-5 border border-[#e5d6a3] bg-[#fffdf8] p-5"><p className="text-sm font-semibold">Continuing your rental.</p><p className="mt-1 text-xs leading-5 text-gray-500">Trade-in is only for vehicle purchases. Your rental continues with eligibility review.</p>{error ? <p className="mt-3 text-xs text-red-700">{error}</p> : null}<button type="button" onClick={() => void continueStep("eligibility", "Your rental is back on the eligibility step. Let’s continue.")} disabled={advanceStep.isPending} className="mt-5 inline-flex items-center gap-2 rounded-full bg-black px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><ArrowRight size={15} /> Continue rental</button></div>;
  ...
  if (transaction.currentStep === "additional_drivers") return <div className="mt-5 border border-[#e5d6a3] bg-[#fffdf8] p-5"><p className="text-sm font-semibold">Additional drivers.</p><p className="mt-1 text-xs leading-5 text-gray-500">You can continue without an additional driver. Any driver you add is saved for separate eligibility and identity review; adding them does not authorize them to operate the vehicle.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="grid gap-1.5 text-xs font-semibold">Legal name<input value={additionalDriver.fullName} onChange={event => setAdditionalDriver(current => ({ ...current, fullName: event.target.value }))} className={fieldClass} autoComplete="name" /></label><label className="grid gap-1.5 text-xs font-semibold">Email <span className="font-normal text-gray-500">(optional)</span><input value={additionalDriver.email} onChange={event => setAdditionalDriver(current => ({ ...current, email: event.target.value }))} className={fieldClass} type="email" autoComplete="email" /></label></div>{error ? <p className="mt-3 text-xs text-red-700">{error}</p> : null}<div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => void submitAdditionalDriver()} disabled={addAdditionalDriver.isPending || additionalDriver.fullName.trim().length < 2} className="inline-flex items-center gap-2 rounded-full border border-black bg-white px-4 py-2.5 text-sm font-semibold text-black disabled:opacity-50">{addAdditionalDriver.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserRound size={15} />} Add for review</button><button type="button" onClick={() => void continueStep("membership", "Next, we’ll review your membership path.")} disabled={advanceStep.isPending} className="inline-flex items-center gap-2 rounded-full bg-black px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><ArrowRight size={15} /> Continue</button></div></div>;
